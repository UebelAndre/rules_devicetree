...
"""Analysis tests for `default_copts` and `copts`."""

load("@bazel_skylib//lib:unittest.bzl", "analysistest", "asserts")

_TOOLCHAIN = "//devicetree/tests:copts_toolchain"

def _preprocess_action(env):
    """Returns the action that runs the C preprocessor on the source file.

    The preprocessor writes the `.tmp.dts`/`.tmp.dtso` file that `dtc` then
    consumes, so it is identified by the extension of its output.
...
copts_test = analysistest.make(
    ...
    config_settings = {
        ...
        # the target under test resolves to a toolchain with `default_copts`.
