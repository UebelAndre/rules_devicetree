...
devicetree_toolchain(<a href="#devicetree_toolchain-name">name</a>, <a href="#devicetree_toolchain-default_copts">default_copts</a>, <a href="#devicetree_toolchain-default_dtcopts">default_dtcopts</a>, <a href="#devicetree_toolchain-dtc">dtc</a>, <a href="#devicetree_toolchain-fdtoverlay">fdtoverlay</a>, <a href="#devicetree_toolchain-preprocess">preprocess</a>)
</pre>
...
| <a id="devicetree_toolchain-default_copts"></a>default_copts |  Default list of flags to the C preprocessor.<br><br>These are only used if source files are preprocessed. They are added after the flags that `dtb()` and `dtbo()` always pass (`-undef -D__DTS__` and the include directories from `deps`), and before `dtb(copts=)` and `dtbo(copts=)`.   | List of strings | optional |  `[]`  |
